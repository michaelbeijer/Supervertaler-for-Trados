---
description: How memory banks integrate with AI translations and chat
---

# AI Integration

Memory banks are automatically integrated into all AI-powered features in Supervertaler. When you translate (batch or chat), the AI consults the active memory bank before producing a translation.

## What the AI loads

Before every translation, Supervertaler reads the active memory bank and loads the most relevant articles:
...
Only articles from the **active** memory bank are loaded. If you keep separate banks per client or domain, switch to the relevant one from the toolbar dropdown before translating.

## How it works with existing context

A memory bank adds an extra intelligence layer on top of the context you already use:

| Context source | What it provides | How the memory bank enhances it |
|---|---|---|
...
All of these work together. Termbases give the AI the terms; the memory bank tells it _why_ those terms were chosen and what to watch out for.

## Memory-aware chat

The Supervertaler Assistant chat window is also memory-aware. When you ask the AI a question about a translation, it has access to the active memory bank alongside the document context, terminology, and TM matches. This means you can ask questions like "What register should I use for this client?" and the AI answers based on your actual KB articles, not generic assumptions.

## Token budget

To avoid overloading the AI's context window, memory bank context is allocated a token budget (approximately 4000 tokens). If the active bank contains more relevant content than fits in the budget, articles are prioritised: client profile first, then domain knowledge, then style guide, then terminology articles.
...
* **Use memory bank when generating prompts (AutoPrompt)** -- enables KB context when AutoPrompt generates new translation prompts
